...
- **Auto-target on task selection**: `useEffect` watches `selectedTaskId` + `nodeMap`. If selected node is `in_progress`, sets `targetNodeId` automatically. Clearing selection or selecting non-in_progress resets it. Footer banner shows targeting status.
- **queue_message parsing**: Split on `\n`, filter `## ` section headers, regex `^\[([^\]]+)\] (.*)# OpenGraft Project Memory
...
4. No synchronous mutable APIs — fire-and-forget + WS observe
 ...
 to extract type + text. Map `child_complete` → `task_completed`, `user` → `user_prompt`, others → `queue_message`. Fall back to raw text if no lines match.
- **TaskDetail props**: Removed `isTargeted`/`onSetTarget`/`running` props — targeting is now implicit via auto-target effect, no button needed.
