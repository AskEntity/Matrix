...
describe("analyzeCacheMisses", () => {
	...
	test("estimated=true usage does not appear in misses or totalUsageEvents", () => {
		const jsonl = buildJsonl([
			{
				type: "usage",
				taskId: "t",
	...
	test("estimated usage does NOT update prevUsageTs — gap crosses over it", () => {
		const jsonl = buildJsonl([
			{
				type: "usage",
				taskId: "t",
			...
			// estimated row in the middle — should be fully transparent
			{
				type: "usage",
				taskId: "t",
				inputTokens: 17_056,
		...
		const r = analyzeCacheMisses(jsonl);
		expect(r.misses).toHaveLength(2);
		expect(r.totalUsageEvents).toBe(2);
		// Gap is from the first REAL usage (ts=1000) to the second REAL usage
		// (ts=10000) — 9000ms. Not 5000 (which would mean the estimated one
		// wrongly updated prevUsageTs).
		expect(r.misses[1]?.gapMs).toBe(9_000);
	});

	test("compact_marker → estimated usage → real usage: compactInGap=true (flag survives)", () => {
		// The exact production trace that motivated this fix:
		//   19192 compact_marker
		//   19194 usage estimated=true (must be transparent)
		//   19196 usage inp=45236 cr=0  ← MUST be compactInGap=true
		const jsonl = buildJsonl([
			{
				type: "usage",
				taskId: "t",
			...
			{ type: "session_config", taskId: "t", ts: 2_200 },
			{
				type: "usage",
				taskId: "t",
				inputTokens: 45_236,
				cacheReadTokens: 0,
				cacheCreationTokens: 45_233,
				outputTokens: 10,
				contextWindow: 200_000,
				ts: 3_000,
			},
		...
		const r = analyzeCacheMisses(jsonl);
		// First usage is 93.75% — hit, not a miss. Second real usage is cr=0 — miss.
		expect(r.misses).toHaveLength(1);
		const postCompact = r.misses[0];
		if (!postCompact) throw new Error("expected post-compact miss");
		expect(postCompact.compactInGap).toBe(true);
		expect(postCompact.stoppedInGap).toBe(false);
	});

	test("agent_stopped → estimated usage → real usage: stoppedInGap=true (flag survives)", () => {
		const jsonl = buildJsonl([
			{
				type: "usage",
				taskId: "t",
