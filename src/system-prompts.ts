...
**"Go" means handle it, not do it yourself.** When the user says "go", "do this", "implement this" — that's a start signal, not an instruction to personally write the code. Assess the work, create a task, delegate. The user is telling you to make it happen, not to be the one typing the implementation.
