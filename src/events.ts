...
export type Event = (
	...
	| {
			/**
			 * Rollback boundary marker — signals that everything between the
			 * target event (referenced by this event's parentEid) and this
			 * marker should be skipped during event reconstruction.
			 *
			 * The parentEid of this event points to the TARGET event (the user
			 * message to roll back to), NOT to the immediately preceding event
			 * in the file. This "jump" in the parentEid chain is what makes
			 * rolled-back events invisible to the chain-walk algorithm.
			 */
			type: "rollback_marker";
			/** eid of the event we're rolling back to (same as this event's parentEid). */
			targetEid: string;
			taskId: string;
			ts: number;
	  }
	 ...
	 * Auto-generated by EventStore on append — callers never set this.
	 * Normally forms a linear chain (each event → previous). A rollback_marker
	 * sets parentEid to the TARGET event, skipping over rolled-back events.
	 */
 ...
 *
 * WITHOUT any rollback_marker, every event chains linearly (each parentEid
 * points to the previous event) → the walk visits every event in order,
 * identical to the old `readActive` behavior.
 *
 * WITH a rollback_marker, its parentEid jumps back to the target event,
 * skipping all rolled-back events between the target and the marker.
 *
