...
describe("task-index (FTS keyword search)", () => {
	...
	test("indexes result + lessons per round, with correct round provenance", () => {
		...
		const lesson = searchIndex(dbPath, "mutationtest");
		const lh = lesson.find((h) => h.field === "lessons");
		expect(lh?.roundIndex).toBe(1);
		expect(lh?.snippet).toContain("mutationtest");

		const firstLesson = searchIndex(dbPath, "smallsample");
		expect(firstLesson.find((h) => h.field === "lessons")?.roundIndex).toBe(0);
	});
