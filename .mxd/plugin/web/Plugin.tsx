...
function ProductionModePage() {
	return (
		<div style={{
			...
			<div style={{ display: "flex", gap: 48, alignItems: "center", maxWidth: 700 }}>
				<div style={{ textAlign: "center" }}>
					...
					<div style={{ fontSize: 28, fontWeight: 700, marginBottom: 8 }}>Matrix</div>
					<div style={{ color: "var(--text-muted, #8b949e)", fontSize: 14 }}>
						Autonomous agent orchestration
					</div>
				</div>
				<div style={{ color: "var(--text-secondary, #b1bac4)", fontSize: 15, lineHeight: 1.6 }}>
					Matrix is running in production mode. Please select a different project
				</div>
			</div>
		</div>
	);
...
export function Plugin({ projectId, productionMode }: { projectId: string; productionMode?: boolean }) {
	if (!projectId)
		return (
			<div style={{ padding: 20, color: "#8b949e" }}>No project selected</div>
		);
	if (productionMode)
		return <ProductionModePage />;
	return (
		<LocaleProvider>
			<ErrorBoundary>
