...
export function registerRoutes(app: Hono, ctx: RuntimeContext) {
	...
	// ── Rollback endpoint ──
	// Roll back a task's session to a specific user message. Stops the
	// running agent, appends a rollback_marker whose parentEid jumps to the
	// target event, then restarts the agent from that point.
	app.post("/projects/:id/tasks/:nodeId/rollback", async (c) => {
		const projectId = c.req.param("id");
		const nodeId = c.req.param("nodeId");
		const body = await c.req.json<{ targetEid: string }>().catch(() => null);

		if (!body?.targetEid) {
			return c.json({ error: "targetEid is required" }, 400);
		}
		...
		const targetEvent = allEvents.find(
			(e) => e.eid === body.targetEid,
		);
		if (!targetEvent) {
			return c.json({ error: "targetEid not found" }, 400);
		}
		...
		if (
			...
			return c.json(
				{ error: "targetEid must point to a user message" },
				400,
			);
		}
		// Must be after the last compact_marker (can't roll back across compact)
		...
		if (lastCompactIdx >= 0 && targetIdx <= lastCompactIdx) {
			return c.json(
				{ error: "Cannot roll back before compact boundary" },
				400,
			);
		}
		...
		if (node.session) {
			...
			// Cover the launchingNodes gap (loop running, session not yet set)
		...
		// Append rollback_marker — parentEid jumps to the target
		...
		// Restart the agent — it will resume from the rolled-back state
		// by reading the chain-walked active events (which exclude the
		// rolled-back region).
		...
		const msg = createUserMessage(
			"Session rolled back. Continue from this point.",
		);
