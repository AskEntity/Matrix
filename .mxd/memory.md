...
- **`src/task-index.ts`** (NEW src/ leaf) — per-project SQLite (bun:sqlite, zero deps) FTS5 index over
  every task's title, description, and each done() round's result + lessons, at **per-field +
...
### Tests
- `src/task-index.test.ts` (15 unit): schema/vec-reservation, title/description/result/lessons provenance,
