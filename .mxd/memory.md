...
## ScrollTarget Sign Bug Hotfix (2026-04-05)

**Regression from commit 9449b55** (scroll refactor). `applyScrollTarget` used `scrollTop = offsetTop - offsetPx`, but `selectTopAnchor` and the jump handler both define `offsetPx = containerTop - entryTop` (viewport coords). Those two computation sites agreed; `applyScrollTarget` used the opposite sign → every reapply shifted scroll in the wrong direction.

**Symptoms**: scroll jumping around, cannot scroll up, stuck at bottom. Each real user scroll captured an anchor, target effect fired reapply, wrong sign snapped to a bad position. MutationObserver (text_delta streaming) made the drift continuous. User-reported 20:45 2026-04-05.

**Fix**: `scrollTop = offsetTop + offsetPx`. Extracted as pure function `anchoredScrollTopPure(offsetTop, offsetPx)` with a regression-guard test asserting the exact formula plus round-trip invariant with `selectTopAnchor`.

**Test lesson**: the original test suite tested `selectTopAnchor` alone AND `applyScrollTarget` alone, but never asserted **round-trip composition** (capture → apply → original position). Two correct-in-isolation functions can still disagree on convention. Always add a round-trip test when two functions are meant to invert each other.
