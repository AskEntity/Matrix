...
- OpenAI/Codex MCP param audit: provider-side emitted tool schemas are correct on both Chat Completions and Responses (optional strings stay optional; booleans stay boolean). The fragile spot is after provider return: OpenAI paths JSON.parse string arguments and executeTool used to forward values verbatim, unlike Anthropic which delivers structured input. Added normalizeToolInput in executeTool to strip empty-string fields before handlers so optional params behave as omitted even if OpenAI/Codex emits "". This fixes update_task-style empty optional arg issues defensively. Boolean string values like "false" are still an upstream/provider serialization issue; not globally coerced here because blind string->boolean conversion across all tools would be riskier without a narrower repro.
