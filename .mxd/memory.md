...
**Destructive tools check `requireSubtreePermission` at handler entry**: `update_task`'s `status`,
`draft` and `parentId` — its prose fields are deliberately ungated, see the next section —
`close_task`, `delete_task`, `reset_task`, and the three folder tools. **Upstream errors are classified before they reach a user** — `classifyUpstreamError` maps
...
**`update_task`'s `title`, `description` and `color` are ungated — settable on ANY node, anywhere
in the tree. `status`, `draft` and `parentId` keep the subtree check.** It gated everything until
2026-07-29, and why that was wrong is the entry.
