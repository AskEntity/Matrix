...
使用 Anthropic 的显式 cache breakpoints 降低 API 成本。每次 API 调用设置 3 个 cache 断点：
1. System prompt（最后一个 block 加 `cache_control`）
2. Tools 数组最后一个 tool definition
3. 对话历史中倒数第二条 user message（`addMessagesCacheControl()`）
...
```
Worker Tools（DirectProvider 内置）：
...
Orchestration Tools（MCP server "opengraft"，10 tools）：
  ...
  report_to_parent(message) → sends child_report to parent queue
  delete_task(taskId) → cleanup worktree + branch + node
  clarify(question) → non-blocking, answer via yield()
...
**隐式 yield**：当 agent 的回复没有 tool call（end_turn）时：
- 如果有 running children → 自动等待 queue 消息（等同于 yield()）
- 如果没有 running children 且没调 done() → 警告，默认 success

**report_to_parent**：子 agent 可以向父 agent 发送非阻塞进度汇报。父 agent 在下次 yield() 时收到 `child_report` 消息。
...
**report_to_parent(message)**：非阻塞式。子 agent 向父 agent 发送进度汇报。父 agent 在下次 yield() 时收到 `child_report` 消息。用于重要的中间结果、阻塞点、状态更新。

**技术上卡住**：agent 调用 `done("failed", reason)` 返回父 agent。父 agent 有更多上下文，可以 resume（给指示）或 reset（换方案）。
